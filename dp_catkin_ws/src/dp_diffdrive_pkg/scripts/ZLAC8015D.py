...
class ZLAC8015D:
	...
	def __init__(self):
		...
		##############
		## 8 inches wheel
		self.travel_in_one_rev = 0.534070751
		self.cpr = 4096
		self.R_Wheel = 0.085
	...
	def deg_to_32bitArray(self, deg):
		...
		dec = int(self.map(deg, -5760, 5760, -65536, 65536)) #5760/16=360
	...
	def set_rpm(self, L_rpm, R_rpm):
		"""Sets motors RPM in (rev/min) in velocity control mode"""
		if L_rpm > 3000:
			L_rpm = 3000
		elif L_rpm < -3000:
			L_rpm = -3000
	...
	def set_rpm_single(self, rpm, isLeft):
		"""Sets single motor RPM in (rev/min) in velocity control mode"""		
		if rpm > 3000:
			rpm = 3000
		elif rpm < -3000:
			rpm = -3000
		...
		if isLeft:
			in_bytes = self.int16Dec_to_int16Hex(rpm)
			result = self.client.write_register(self.L_CMD_RPM, in_bytes, unit=self.ID)
		else:
			rpm = -rpm
			in_bytes = self.int16Dec_to_int16Hex(rpm)
			result = self.client.write_register(self.R_CMD_RPM, in_bytes, unit=self.ID)

	def get_rpm(self): 
		"""Returns RPM in (rev/min) in velocity control mode only"""
	...
	def set_relative_angle(self, ang_L, ang_R):
		...
		R_array = self.deg_to_32bitArray(-ang_R) #!!! reversed direction
